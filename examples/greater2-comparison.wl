...
CT`CacheClear[];
tGRT1 = First@RepeatedTiming[GRT`ChristoffelFromMetric[g]];
grtChris = CT`Components[GRT`ChristoffelFromMetric[g]];
...
chrisAgree = CT`EquivalentQ[grtChris, gr2Chris];
...
CT`CacheClear[];
tGRT2 = First@RepeatedTiming[GRT`RiemannFromMetric[g]];
grtRiem = CT`Components[GRT`RiemannFromMetric[g]]; (* R_{abcd} all down *)
...
riemAgree = CT`EquivalentQ[grtRiem, gr2RiemDown];
...
CT`CacheClear[];
tGRT3 = First@RepeatedTiming[GRT`RicciFromMetric[g]];
grtRicci = CT`Components[GRT`RicciFromMetric[g]];

clearGR2Cache[];
tGR2c3 = First@RepeatedTiming[GREATER2`Ricci[gdown, coords]];
gr2Ricci = GREATER2`Ricci[gdown, coords];

ricciAgree = CT`EquivalentQ[grtRicci, gr2Ricci];
...
CT`CacheClear[];
tGRT4 = First@RepeatedTiming[GRT`RicciScalarFromMetric[g]];
grtR = GRT`RicciScalarFromMetric[g];

clearGR2Cache[];
tGR2c4 = First@RepeatedTiming[GREATER2`SCurvature[gdown, coords]];
gr2R = GREATER2`SCurvature[gdown, coords];

rAgree = CT`EquivalentQ[grtR, gr2R];
...
grtRiemObj = GRT`RiemannFromMetric[g]; (* all-down CTensor *)
tGRT5 = First@RepeatedTiming[CT`Raise[grtRiemObj, 1, g]];
grtRaised = CT`Components[CT`Raise[grtRiemObj, 1, g]]; (* R^a_{bcd} *)
...
raiseAgree = CT`EquivalentQ[grtRaised, gr2Riem];
...
grtRiemUp = CT`Raise[grtRiemObj, 1, g];
tGRT6 = First@RepeatedTiming[CT`Trc[grtRiemUp, {1, 3}]];
grtTraced = CT`Components[CT`Trc[grtRiemUp, {1, 3}]];
...
traceAgree = CT`EquivalentQ[grtTraced, gr2Traced];
Print["  GRT:      ", 1000 tGRT6, " ms"];
Print["  GREATER2: ", 1000 tGR2c6, " ms"];
Print["  agree:    ", traceAgree];
Print["  == Ricci:  ", CT`EquivalentQ[grtTraced, grtRicci]];
...
grtEF = CT`Components[CT`Transform[g, efCoords, efRules]];
...
grtEFok = CT`EquivalentQ[grtEF, efExpected];
gr2EFok = CT`EquivalentQ[gr2EF, efExpected];
...
vComps = {0, 1, 0, 0};
vObj = CT`Tensor[coords, {"up"}, vComps, gdown, <||>];

CT`CacheClear[];
tGRT8 = First@RepeatedTiming[
  GRT`CovariantDFromMetric[vObj, g]];
grtCovD = CT`Components[GRT`CovariantDFromMetric[vObj, g]]; (* nabla_mu v^nu: {mu, nu} *)
...
covDAgree = CT`EquivalentQ[grtCovD, gr2CovD];
...
grtGeod = GRT`GeodesicEquationsFromMetric[g, \[Lambda]];
...
geodAgree = And @@ Table[
  CT`EquivalentQ[Simplify[grtGeod[[i]] - gr2Geod[[i]]], 0],
  {i, 4}];
