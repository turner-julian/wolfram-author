...
| `scalar` | `wl`, `expect`    | `wl`'s value equals `expect` by `Core`EquivalentQ` in a clean kernel — **Wolfram only**, no second engine; scalars only |
...
--skill-name wolfram-author --model <id>`.
...
nowhere else. `Core`EquivalentQ` returns True / False / `$Failed`; only an
